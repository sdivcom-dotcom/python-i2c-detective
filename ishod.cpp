/* I2C Detective - see http://www.technoblogy.com/show?3FFW
...
void list (uint8_t addr, const device_t *me) {
  ...
  #if defined(AVR_DATA)
  Serial.print((__FlashStringHelper *)pgm_read_word(&me->category));
  Serial.print(": "); Serial.print((char *)(pgm_read_word(&me->device)));
  #else
  Serial.print(me->category);
...
void lookup (uint8_t addr) {
  ...
  for (int pass=1; pass<4; pass++) {
    for (int i=0; i<Devices; i++) {
      const device_t *me = &Device[i];
      #if defined(AVR_DATA)
      uint8_t from = pgm_read_byte(&me->from);
      uint8_t to = pgm_read_byte(&me->to);
      uint8_t test = pgm_read_byte(&me->test);
      uint8_t value = pgm_read_byte(&me->value);
      #else
